        ...
        with smart_open(sqanti_file, mode='rt') as f:
            ...
            for line in f:
                (isoform, chrom, strand, length, exons, structural_category, \
                     ...
                     subcategory, RTS_stage, all_canonical, min_sample_cov, min_cov, min_cov_pos, \
                     sd_cov, FL, n_indels, n_indels_junc, bite, iso_exp, gene_exp, ratio_exp, \
                     FSM_class, coding, ORF_length, CDS_length, CDS_start, CDS_end, \
                     CDS_genomic_start, CDS_genomic_end, predicted_NMD, perc_A_downstream_TTS, \
                     seq_A_downstream_TTS, dist_to_CAGE_peak, within_CAGE_peak, dist_to_polyA_site, \
                     within_polyA_site, polyA_motif, polyA_dist, polyA_motif_found, ORF_seq, \
                     ratio_TSS) = line.split("\t")
            ...
            total_counts = FSM_counts + ISM_counts + NIC_counts + NNC_counts + genic_genomic_counts + antisense_counts + fusion_counts + intergenic_counts + genic_intron_counts
    ...
    for sample_name, counts_dict in results.items():
        ...
        for k in sorted_keys:
            if current_count + counts_dict["total_counts"][k] >= one_percent_reads:
                quantile_one_percent = k if k < quantile_one_percent else quantile_one_percent
                break
        ...
        for k in sorted_keys[::-1]:
            if current_count + counts_dict["total_counts"][k] >= one_percent_reads:
                quantile_ninetynine_percent = k if k > quantile_ninetynine_percent else quantile_ninetynine_percent
                break
            current_count += counts_dict["total_counts"][k]
...
def make_read_length_table(results, category):
    all_dfs = []
    for sample_name, counts_dict in results.items():
        df = pd.DataFrame.from_dict(counts_dict[category + "_counts"], orient='index').reset_index(names="read_length")
...
def make_normalized_read_length_table(results, category):
    ...
    for sample_name, counts_dict in results.items():
        df = pd.DataFrame.from_dict(counts_dict[category + "_counts"], orient='index').reset_index(names="read_length")
        df = pd.DataFrame.from_dict({k: v/counts_dict[category + "_sum"] for (k, v) in counts_dict[category + "_counts"].items()}, orient='index').reset_index(names="read_length")
        df["sample_name"] = sample_name
        all_dfs.append(df)
    full_df = pd.concat(all_dfs)
    return full_df
...
def make_total_normalized_read_length_table(results, category):
    ...
    for sample_name, counts_dict in results.items():
        df = pd.DataFrame.from_dict(counts_dict[category + "_counts"], orient='index').reset_index(names="read_length")
        df = pd.DataFrame.from_dict({k: v/counts_dict["total_sum"] for (k, v) in counts_dict[category + "_counts"].items()}, orient='index').reset_index(names="read_length")
        df["sample_name"] = sample_name
        all_dfs.append(df)
    full_df = pd.concat(all_dfs)
    return full_df
...
def plot_categories_histogram(results,
                              output_handle = None,
                              categories_to_plot = ["FSM", "ISM", "NIC", "NNC", "genic_genomic", "antisense", "fusion", "intergenic", "genic_intron"],
    ...
    for i, (sample_name, counts_dict) in enumerate(results.items()):
        ...
        for j, category in enumerate(categories_to_plot):
            col_height[(n_samples * j) + i] = counts_dict[category + "_sum"] / counts_dict["total_sum"] * 100
            col_placement[(n_samples * j) + i] = (i * col_width) + (j * categories_group_spacing)
    
    fig, ax = plt.subplots(figsize=(24, 12))
    
    ax.bar(col_placement, col_height, width=col_width, facecolor=col_colors, edgecolor="white", linewidth=(0.7*col_width))
    ax.set_xlim(-1, (categories_group_spacing * n_categories) - categories_spacing)
    ax.set_xticks(np.arange(((((n_samples-1) * col_width))/2), (categories_group_spacing * n_categories), categories_group_spacing))
    ax.set_ylim(0, 100)
    ...
    ax.set_title("Reads Distribution Across Structural Categories and Samples", fontsize=40)
    ax.legend(handles=[matplotlib.patches.Rectangle([0,0], 2, 1, facecolor=i) for i in palette], labels=col_labels, facecolor="white")
...
def plot_subcategories_histogram(results,
                                 category_to_plot,
                                 output_handle = None,
    ...
    for i, (sample_name, counts_dict) in enumerate(results.items()):
        ...
        for j, category in enumerate(subcategories_to_plot):
            col_height[(n_samples * j) + i] = counts_dict[category].total() / counts_dict[category_to_plot + "_sum"] * 100
            col_placement[(n_samples * j) + i] = (i * col_width) + (j * subcategories_group_spacing)
    
    fig, ax = plt.subplots(figsize=(24, 12))
    
    ax.bar(col_placement, col_height, width=col_width, facecolor=col_colors, edgecolor="white", linewidth=(0.7*col_width))
    ax.set_xlim(-1, (subcategories_group_spacing * n_categories) - subcategories_spacing)
    ax.set_xticks(np.arange(((((n_samples-1) * col_width))/2), (subcategories_group_spacing * n_categories), subcategories_group_spacing))
    ax.set_ylim(0, 100)
    ax.set_yticks(np.arange(0, 101, 10))
    ax.set_yticks(np.arange(0, 101, 1), minor=True)
    ax.set_yticklabels(np.arange(0, 101, 10), fontsize=20)
    ...
    ax.set_title("Reads Distribution Across " + category_to_plot + " Subcategories and Samples", fontsize=40)
    ax.legend(handles=[matplotlib.patches.Rectangle([0,0], 2, 1, facecolor=i) for i in palette], labels=col_labels, facecolor="white")
    # ax.legend(handles=[matplotlib.patches.Rectangle([0,0], 2, 1, facecolor=i) for i in col_colors], labels=col_labels, facecolor="white")
...
def plot_read_length_distribution_ridgeline(df, 
                                            min_x, 
                                            max_x,
                                            output_handle = None, 
                                            share_y_axis = False, 
    ...
    g = sns.FacetGrid(df, row="sample_name", hue="sample_name", aspect=scaled_aspect, height=scaled_height, # palette=custom_palette,
                      legend_out=False, margin_titles=False, sharey=share_y_axis)
    ...
    g.map(weighted_hist, "read_length", 0,  binwidth=20, binrange=(min_x, max_x), 
          common_bins=True, log_scale=False, kde=True, 
          kde_kws={"clip": (min_x, max_x), "bw_adjust": 0.2}, line_kws={"alpha": 0.5})
    ...
    def label(x, color, label):
        ax = plt.gca()
        ax.text(1.02, .01, label, fontsize=scaled_fontsize*0.8, fontweight="bold", color=color,
...
def plot_read_length_distribution_ridgeline_manual_kde(df, 
                                                       min_x, 
                                                       max_x, 
                                                       output_handle = None, 
                                                       share_y_axis = False, 
    ...
    g.map(weighted_hist, "read_length", 0,  binwidth=20, binrange=(min_x, max_x), alpha=0.5,
          common_bins=True, log_scale=False, kde=False)
    
    # Manually computed and scaled KDE
    def manual_scaled_kde(data, color, min_x, max_x, binwidth, bw_adjust, grid_points, **kwargs):
    ...
    g.map_dataframe(manual_scaled_kde, min_x=min_x, max_x=max_x, binwidth=20, bw_adjust=0.2, grid_points=gridpoints)
...
def plot_trend_line_only(df, 
    ...
    g = sns.FacetGrid(df, row="sample_name", hue="sample_name", aspect=(2*n_samples), height=(10/n_samples), # palette=custom_palette,
                      legend_out=False, margin_titles=False, sharey=share_y_axis)
    g.fig.patch.set_facecolor('none')
    for ax in g.axes.flat:
        ax.patch.set_facecolor('none')
    ...
    g.map(weighted_hist, "read_length", 0, binwidth=20, binrange=(min_x, max_x), 
          common_bins=True, log_scale=False, kde=True, fill=False, linewidth=0,
          kde_kws={"clip": (min_x, max_x), "bw_adjust": 0.2}, line_kws={"alpha": 0.5, "linewidth": 5})
    ...
    g.add_legend(handles=[matplotlib.patches.Rectangle([0,0], 2, 1, facecolor=i) for i in g._colors], labels=g.hue_names, fontsize=16, facecolor="white")
...
def plot_trend_line_manual_kde(df, 
                               min_x, 
                               max_x, 
                               output_handle = None, 
                               share_y_axis = True, 
    ...
    def weighted_hist(x, weights, **kwargs):
        sns.histplot(x=x, weights=weights, binwidth=20, binrange=(min_x, max_x),
                     common_bins=True, log_scale=False, kde=False, fill=False, linewidth=0)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Associate classification files with sample names.")
    parser.add_argument("--classification_files", required=True,
    ...
    parser.add_argument("--output", required=True,
                        help="Output file or output string PREFIX.")
    parser.add_argument("--type", required=True,
                        help="Type of classification or output (as a string).")
...
def main():
    ...
    if len(classification_files) != len(sample_names):
        raise ValueError("The number of classification files must match the number of sample names.")
    
    sample_dict = {sample: file for sample, file in zip(sample_names, classification_files)}
    ...
    if args.type == "sqanti":
        ...
        read_length_categories = ["total", "FSM", "ISM", "NIC", "NNC", "antisense", "fusion", "genic_genomic", "genic_intron", "intergenic"]
        distribution_categories = ["FSM", "ISM", "NIC", "NNC", "antisense", "fusion", "genic_genomic", "genic_intron", "intergenic"]
    ...
    plot_categories_histogram(
        ...
        categories_to_plot = distribution_categories
    )
    for category in with_subcategories:
        plot_subcategories_histogram(
            results, 
            output_handle = pdf_handle, 
            category_to_plot = category
        )
    pdf_handle.close()
